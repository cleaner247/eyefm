...
def test_formal_attention_and_mask_are_explicit() -> None:
    ...
    assert bert["mask"]["mode"] == "paired_dual_scale"
    assert bert["mask"]["dual_short_num_blocks"] == 15
    assert bert["mask"]["dual_long_num_blocks"] == 5
    assert bert["train"]["total_steps"] == 20000
    assert bert["train"]["max_trials_per_gpu"] == 64
    assert tokenizer["encoder"]["n_layers"] == bert["bert"]["n_layers"] == 12
...
def test_formal_recipe_uses_unified_k16_task_bags() -> None:
    ...
    assert pd3["mil"]["trials_per_task"] == 16
    assert pd3["mil"]["eligibility_min_trials_per_task"] == 16
    assert pd3["mil"]["evaluation_min_trials_per_task"] == 4
...
def test_formal_recipe_uses_v6_and_v6_derived_artifacts() -> None:
    ...
    assert "eyemae_fast_dataset_v6" in pd3["data"]["data_dir"]
...
def test_quality_gate_fails_closed(tmp_path: Path) -> None:
    ...
    pipeline.recipe["quality_gates"]["mode"] = "strict"
