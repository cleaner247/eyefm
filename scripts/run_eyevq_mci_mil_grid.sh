...
GRID_ROOT="${GRID_ROOT:-outputs/eyevq/downstream_mci_subject_mil_grid_seed42}"
EPOCHS="${EPOCHS:-80}"
PATIENCE="${PATIENCE:-81}"
...
for unfrozen_layers in 6 8 10 12; do
  ...
  for encoder_lr in 2e-6 5e-6 1e-5; do
    lr_tag="${encoder_lr//-/m}"
    output_dir="$GRID_ROOT/unfreeze${unfrozen_layers}_enc${lr_tag}_e${EPOCHS}"
