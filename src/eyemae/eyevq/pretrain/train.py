...
from eyemae.eyevq.manual_features import ManualFeatureTargetStore
...
def load_code_ids_cache(
    ...
    *,
    ...
    target_type: str = "joint_code",
):
    """Load the immutable offline code-ID or normalized-latent target cache."""
    ...
    with np.load(path, allow_pickle=True) as d:
        ...
        # Older discrete caches predate the explicit target_type metadata; the
        # same integer IDs are valid for joint and factorized CE.
        cached_target_type = str(d["target_type"]) if "target_type" in d else target_type
        ...
        key = "latent_targets" if target_type == TARGET_NORMALIZED_LATENT else "code_ids"
        if key not in d:
            raise ValueError(f"Target cache does not contain required array {key!r}")
        codes = np.asarray(d[key])
    ...
    if cached_target_type != target_type:
        raise ValueError(
            f"Target cache type={cached_target_type!r}, expected {target_type!r}"
        )
    ...
    expected_ndim = 4 if target_type == TARGET_NORMALIZED_LATENT else 3
    if codes.ndim != expected_ndim or codes.shape[2] != 2 or codes.shape[0] != len(gids):
...
def lookup_code_ids(
    batch, gid_to_idx, codes_arr, device, max_patches, *, target_type="joint_code"
):
    ...
    dtype = torch.float32 if target_type == TARGET_NORMALIZED_LATENT else torch.long
...
def train_bert(
    ...
    *,
    ...
    uses_direct_reconstruction = target_type == TARGET_DIRECT_RECONSTRUCTION
    uses_code_targets = target_type not in {
        TARGET_RAW_PATCH,
        TARGET_DIRECT_RECONSTRUCTION,
    }
    uses_latent_targets = target_type == TARGET_NORMALIZED_LATENT
    regression_target = target_type in {
        TARGET_RAW_PATCH,
        TARGET_DIRECT_RECONSTRUCTION,
        TARGET_NORMALIZED_LATENT,
    }
    ...
    if rank == 0:
        ...
        elif uses_direct_reconstruction:
            manual_cfg = cfg.get("manual_features", {})
            dependencies = {
                f"manual_features.{key}": manual_cfg[key]
                for key in (
                    "cache_path",
                    "stats_path",
                    "task_metric_weight_path",
                )
            }
    ...
    if uses_code_targets:
        ...
        logger.info(
            "Target: %s; tokenizer and code-ID cache are not used",
            target_type,
        )

    manual_feature_store = (
        ManualFeatureTargetStore(
            cfg["manual_features"], cfg["loss"], device
        )
        if uses_direct_reconstruction
        else None
    )
    if is_rank0() and manual_feature_store is not None:
        logger.info(
            "Direct reconstruction supervision: eye decode loss + %dD manual-feature loss; no accuracy metric",
            manual_feature_store.num_features,
        )
    ...
    def run_validation(model_eval, step):
        ...
        per_dim_ce_sum = None
        dim_dist_sum = None
        direct_stats_sum: dict[str, float] = {}
        total_masked = 0
        ...
        for val_batch in val_loader:
            ...
            v_manual = (
                manual_feature_store.lookup(val_batch)
                if manual_feature_store is not None
                else None
            )
            if mask_mode == PAIRED_DUAL_SCALE:
                short_mask, _ = generate_eyemae_mask_paired_fixed_blocks(
                    v_valid, v_pad, num_blocks=dual_short_blocks,
                    span_min=dual_short_min, span_max=dual_short_max,
                    min_gap=dual_min_gap, generator=val_mask_generator,
                )
                long_mask, _ = generate_eyemae_mask_paired_fixed_blocks(
                    v_valid, v_pad, num_blocks=dual_long_blocks,
                    span_min=dual_long_min, span_max=dual_long_max,
                    min_gap=dual_min_gap, generator=val_mask_generator,
                )
                v_mask = torch.cat([short_mask, long_mask], dim=0)
                v_span_lengths = None
                v_content = torch.cat([v_content, v_content], dim=0)
                v_stim = torch.cat([v_stim, v_stim], dim=0)
                v_quality = torch.cat([v_quality, v_quality], dim=0)
                v_pad = torch.cat([v_pad, v_pad], dim=0)
                v_nm = torch.cat([v_nm, v_nm], dim=0)
                v_task = torch.cat([v_task, v_task], dim=0)
                if v_code_ids is not None:
                    v_code_ids = torch.cat([v_code_ids, v_code_ids], dim=0)
                if v_manual is not None:
                    for key in ("targets", "loss_mask", "weights"):
                        v_manual[key] = torch.cat(
                            [v_manual[key], v_manual[key]], dim=0
                        )
            else:
                v_mask_output = generate_eyemae_mask(
                    v_valid, v_pad, mask_ratio, mode=mask_mode,
                    generator=val_mask_generator, span_min=mask_span_min,
            ...
            eligible_views = 2 if mask_mode == PAIRED_DUAL_SCALE else 1
            v_eligible_pairs = int(
                paired_time_eligibility(v_valid, v_pad[:v_valid.shape[0]]).sum().item()
            ) * eligible_views
            ...
            v_loss, v_stats = model_eval(stim_patches=v_stim, eye_patches=v_content,
                                          ...
                                          eye_code_ids=None if uses_latent_targets else v_code_ids,
                                          eye_latent_targets=v_code_ids if uses_latent_targets else None,
                                          bert_mask=v_mask,
                                          mask_span_lengths=v_span_lengths,
                                          manual_feature_targets=(
                                              v_manual["targets"] if v_manual else None
                                          ),
                                          manual_feature_loss_mask=(
                                              v_manual["loss_mask"] if v_manual else None
                                          ),
                                          manual_feature_binary_mask=(
                                              v_manual["binary_mask"] if v_manual else None
                                          ),
                                          manual_feature_count_mask=(
                                              v_manual["count_mask"] if v_manual else None
                                          ),
                                          manual_feature_pos_weight=(
                                              v_manual["pos_weight"] if v_manual else None
                                          ),
                                          manual_feature_bce_scale=(
                                              v_manual["bce_scale"] if v_manual else None
                                          ),
                                          manual_feature_weights=(
                                              v_manual["weights"] if v_manual else None
                                          ))
            ...
            if n_masked > 0 and not uses_direct_reconstruction:
                total_val_acc += (
                    float(torch.as_tensor(v_stats["bert_acc"]).item()) * n_masked
                )
            if uses_direct_reconstruction:
                for key in (
                    "L_eye", "L_feat", "L_total", "L_eye_weighted",
                    "L_feat_weighted", "xy_loss", "area_loss",
                    "blink_loss", "vel_loss", "L_feat_binary",
                    "L_feat_continuous",
                ):
                    if key in v_stats:
                        direct_stats_sum[key] = direct_stats_sum.get(key, 0.0) + (
                            float(torch.as_tensor(v_stats[key]).item())
                            * n_supervised_trials
                        )
        ...
        if uses_direct_reconstruction:
            for key, value in direct_stats_sum.items():
                res[f"val/{key}"] = value / max(total_supervised_trials, 1)
        elif uses_latent_targets:
            res["val/acc"] = avg_acc
            res["val/mse"] = avg_loss
            res["val/cosine"] = avg_acc
        else:
            res["val/acc"] = avg_acc
            res["val/perplexity"] = math.exp(avg_loss)
    ...
    if mask_mode not in {PAIRED_RANDOM, PAIRED_SPAN, PAIRED_MULTIBLOCK, PAIRED_DUAL_SCALE}:
    ...
    dual_short_blocks = int(mask_cfg.get("dual_short_num_blocks", 10))
    dual_short_min = int(mask_cfg.get("dual_short_span_min_patches", 1))
    dual_short_max = int(mask_cfg.get("dual_short_span_max_patches", 3))
    dual_long_blocks = int(mask_cfg.get("dual_long_num_blocks", 4))
    dual_long_min = int(mask_cfg.get("dual_long_span_min_patches", 4))
    dual_long_max = int(mask_cfg.get("dual_long_span_max_patches", 6))
    dual_min_gap = int(mask_cfg.get("dual_min_gap", 1))
    ...
    if is_rank0():
        ...
        if mask_mode == PAIRED_DUAL_SCALE:
            span_text = (
                f"; views=2; short_blocks={dual_short_blocks}"
                f"; short_span=[{dual_short_min},{dual_short_max}]"
                f"; long_blocks={dual_long_blocks}"
                f"; long_span=[{dual_long_min},{dual_long_max}]"
                f"; overlap=false; min_gap={dual_min_gap}; view_loss_weight=0.5/0.5"
            )
        logger.info(
    ...
    for epoch in range(1000):
        ...
        for batch in loader:
            ...
            manual_features = (
                manual_feature_store.lookup(batch)
                if manual_feature_store is not None
                else None
            )

            # ── 2. Mask jointly valid L/R tokens at the same time patches ──
            original_batch_size = int(content.shape[0])
            if mask_mode == PAIRED_DUAL_SCALE:
                short_mask, _ = generate_eyemae_mask_paired_fixed_blocks(
                    eye_valid, pad_mask, num_blocks=dual_short_blocks,
                    span_min=dual_short_min, span_max=dual_short_max,
                    min_gap=dual_min_gap,
                )
                long_mask, _ = generate_eyemae_mask_paired_fixed_blocks(
                    eye_valid, pad_mask, num_blocks=dual_long_blocks,
                    span_min=dual_long_min, span_max=dual_long_max,
                    min_gap=dual_min_gap,
                )
                bert_mask = torch.cat([short_mask, long_mask], dim=0)
                mask_span_lengths = None
                content = torch.cat([content, content], dim=0)
                stim = torch.cat([stim, stim], dim=0)
                quality = torch.cat([quality, quality], dim=0)
                pad_mask = torch.cat([pad_mask, pad_mask], dim=0)
                eye_nonmissing = torch.cat([eye_nonmissing, eye_nonmissing], dim=0)
                task_ids = torch.cat([task_ids, task_ids], dim=0)
                if eye_code_ids is not None:
                    eye_code_ids = torch.cat([eye_code_ids, eye_code_ids], dim=0)
                if manual_features is not None:
                    for key in ("targets", "loss_mask", "weights"):
                        manual_features[key] = torch.cat(
                            [manual_features[key], manual_features[key]], dim=0
                        )
            else:
                mask_output = generate_eyemae_mask(
                    eye_valid=eye_valid, pad_mask=pad_mask, mask_ratio=mask_ratio,
                    mode=mask_mode, span_min=mask_span_min, span_max=mask_span_max,
                    span_length_distribution=mask_span_length_distribution,
                    span_length_power=mask_span_length_power,
                    span_length_probabilities_values=mask_span_length_probabilities,
                    multiblock_num_blocks=mask_multiblock_num_blocks,
            ...
            eligible_pairs_tensor = (
                paired_time_eligibility(
                    eye_valid, pad_mask[:original_batch_size]
                ).sum() * (2 if mask_mode == PAIRED_DUAL_SCALE else 1)
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=use_bf16):
                ...
                loss, stats = model(
                    ...
                    task_ids=task_ids,
                    eye_code_ids=None if uses_latent_targets else eye_code_ids,
                    eye_latent_targets=eye_code_ids if uses_latent_targets else None,
                    bert_mask=bert_mask,
                    mask_span_lengths=mask_span_lengths,
                    manual_feature_targets=(
                        manual_features["targets"] if manual_features else None
                    ),
                    manual_feature_loss_mask=(
                        manual_features["loss_mask"] if manual_features else None
                    ),
                    manual_feature_binary_mask=(
                        manual_features["binary_mask"] if manual_features else None
                    ),
                    manual_feature_count_mask=(
                        manual_features["count_mask"] if manual_features else None
                    ),
                    manual_feature_pos_weight=(
                        manual_features["pos_weight"] if manual_features else None
                    ),
                    manual_feature_bce_scale=(
                        manual_features["bce_scale"] if manual_features else None
                    ),
                    manual_feature_weights=(
                        manual_features["weights"] if manual_features else None
                    ),
                )
            ...
            if not uses_direct_reconstruction:
            ...
            if is_rank0() and should_log:
                ...
                if not uses_direct_reconstruction:
                    reduced_log_values.append(total_acc)
                logged_values = torch.stack(reduced_log_values).tolist()
                masked_pairs, eligible_pairs, loss_sum = logged_values[:3]
                avg_loss = loss_sum / steps_since_log
                avg_acc = (
                    logged_values[3] / steps_since_log
                    if not uses_direct_reconstruction
                    else None
                )
                ...
                parts = [
                    ...
                    f"mask_views={2 if mask_mode == PAIRED_DUAL_SCALE else 1}",
                ...
                if avg_acc is not None:
                    parts.insert(
                        2,
                        f"{'cosine' if uses_latent_targets else 'acc'}={avg_acc:.3f}",
                    )
                if uses_direct_reconstruction:
                    for key in (
                        "L_eye", "L_feat", "L_eye_weighted",
                        "L_feat_weighted", "xy_loss", "area_loss",
                        "blink_loss", "vel_loss",
                    ):
                        if key in stats:
                            parts.append(
                                f"{key}={float(torch.as_tensor(stats[key]).item()):.5f}"
                            )
            ...
            if should_validate:
                ...
                if is_rank0():
                    ...
                    if val_metrics:
                        if uses_direct_reconstruction:
                            _line = (
                                f"  val/loss={val_metrics['val/loss']:.5f} "
                                f"val/L_eye={val_metrics['val/L_eye']:.5f} "
                                f"val/L_feat={val_metrics['val/L_feat']:.5f}"
                            )
                        elif uses_latent_targets:
                            _line = (
                                f"  val/loss={val_metrics['val/loss']:.5f} "
                                f"val/cosine={val_metrics['val/cosine']:.4f}"
                            )
                        else:
                            _line = (
                                f"  val/loss={val_metrics['val/loss']:.5f} "
                                f"val/acc={val_metrics['val/acc']:.4f} "
                                f"val/ppl={val_metrics['val/perplexity']:.1f}"
                            )
    ...
    if is_rank0():
        ...
        if final_val_metrics:
            ...
            if final_improved:
                if "val/acc" in final_val_metrics:
